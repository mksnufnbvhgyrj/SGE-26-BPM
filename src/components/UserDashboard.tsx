...
import { ShieldAlert, Bell, LogOut, ClockIcon, Scale, FileText, FileSpreadsheet, File, Download, FileUp, Briefcase, Phone, Mail, Paperclip } from 'lucide-react';
import { Member, Audiencia } from '../types';
import { getStatusClasses, formatBytes, formatCPF, formatMatricula } from '../utils/constants';
...
const UserDashboard: React.FC<UserDashboardProps> = ({ user, audiencias, onLogout, onMarkNotificationsAsRead }) => {
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4 p-4 bg-slate-50 rounded-xl border border-slate-100">
                  <div>
                    ...
                    <p className="text-slate-900 font-semibold font-mono">{formatMatricula(user.matricula)}</p>
                  </div>
                  <div>
                    <p className="text-xs text-slate-500 font-medium uppercase tracking-wider mb-1">CPF</p>
                    <p className="text-slate-900 font-semibold font-mono">{formatCPF(user.cpf)}</p>
                  </div>
                </div>
