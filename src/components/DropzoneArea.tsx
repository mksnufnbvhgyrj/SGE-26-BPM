...
export const DropzoneArea = ({ onDropFiles }: { onDropFiles: (files: File[]) => void }) => {
  ...
  return (
    <div 
      ...
      className={`border-2 border-dashed rounded-xl p-8 text-center transition-all cursor-pointer min-h-[160px] flex flex-col items-center justify-center ${
        isDragActive ? 'border-blue-500 bg-blue-50 scale-[1.02]' : 'border-slate-300 bg-white hover:bg-slate-50'
      }`}
    >
      <UploadCloud className={`w-12 h-12 mx-auto mb-4 ${isDragActive ? 'text-blue-600' : 'text-slate-400'}`} />
      ...
      <div className="px-6 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium inline-flex items-center gap-2 shadow-sm min-h-[44px]">
        <FolderOpen className="w-4 h-4" /> Procurar Arquivos
      </div>
      <input {...getInputProps()} className="sr-only" />
      <p className="text-xs text-slate-400 mt-4">Formatos: PDF, Word, Excel. Máx: 5MB por arquivo.</p>
    </div>
  );
};
